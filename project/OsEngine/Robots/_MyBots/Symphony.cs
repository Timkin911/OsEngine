...
namespace OsEngine.Robots
{
    ...
    public class Symphony : BotPanel
    {
            ...
            if (RegimeTradingModule.ValueString == "Cointegration")
            {
                for (int i = 0; i < _tabScreener.Tabs.Count; i++)
                {
                    ...
                    else if (StartProgram == StartProgram.IsOsTrader)
                    {
                        ...
                        if (StartProgram == StartProgram.IsOsTrader)
                        {
                            if (candlesCur[candlesCur.Count - 1].TimeStart
                                   != indexCandles[indexCandles.Count - 1].TimeStart)
                            {
                                candlesCur = _tabScreener.Tabs[i].CandlesAll;
                            }
                        }
        ...
        private void _tabToTrade_CandleFinishedEvent(List<Candle> candles, BotTabSimple tab)
        {
            ...
            List<Candle> candlesInIndex = indexCandles;

            if (candlesInIndex == null ||
                candlesInIndex.Count == 0)
            {
                return;
            }
            ...
            {
                _moduleSoldiers.TryTrade(candles, candlesInIndex, tab, _tabScreener);
            }
            else if (RegimeTradingModule.ValueString == "Linear Regression")
            {
                _moduleLinearRegression.TryTrade(candles, candlesInIndex, tab, _tabScreener);
            }
            else if (RegimeTradingModule.ValueString == "Cointegration")
            {
                if (candles[candles.Count - 1].TimeStart != candlesInIndex[candlesInIndex.Count - 1].TimeStart)
                {
                    if (StartProgram == StartProgram.IsOsTrader)
                    {
                        if (candles[candles.Count - 1].TimeStart < candlesInIndex[candlesInIndex.Count - 1].TimeStart)
                        {
                            candlesInIndex = candlesInIndex.GetRange(0, candlesInIndex.Count - 1);
                        }
                ...
                _moduleCointegration.TryTrade(candles, candlesInIndex, tab, _tabScreener);
            }
        }
        ...
        private void CheckSecurities(DateTime time)
        {
            ...
            for (int i = 0; i < _tabScreener.Tabs.Count; i++)
            {
                if (_tabScreener.Tabs[i].Security != null)
                {
                    haveOnePaper = true;
                    break;
                }
            ...
            for (int i = 0; i < topSecuritiesByVolume.Count; i++)
            {
                ...
                if (topSecuritiesByVolume[i].Tab.Security == null)
                {
                ...
                {
                    string logMessag = curTab.Security.Name;
                    logMessag += "   Go: close module";
                    securitiesCloseModule.Add(curTab.Security.Name);
                    logs.Add(logMessag);
                    continue;
                }

                string logMessage = curTab.Security.Name;
                ...
                {
                    securitiesTm1.Add(curTab.Security.Name);
    ...
    public class TradeModuleCointegration
    {
            ...
            decimal correlationLast)
        {
            ...
            if (TimeDeleyFilterIsOn.ValueBool == true)
            {
                ...
                {
                    if (TimeLastCloseByBadCorrelationArray[i].SecurityName == tab.Security.Name)
                    {
                    ...
                    && curPos.ProfitPortfolioPunkt < 0)
                {
                    ...
                    if (TimeDeleyFilterIsOn.ValueBool == true)
                    {
                        ...
                        if (myTime == null)
                        {
                            ...
                            myTime.SecurityName = tab.Security.Name;
        ...
        public decimal GetVolume(BotTabSimple tab)
        {
            ...
            if (tab.StartProgram == StartProgram.IsOsTrader) // "% от депозита для Бинанс или MOEX"
            {
                ...
                return Math.Round((AllPortfolioValue * (VolumeOnPositionMain.ValueDecimal / 100)) / contractPrice / tab.Security.Lot,
                   tab.Security.DecimalsVolume);
            }
            else if (tab.StartProgram == StartProgram.IsTester
                || tab.StartProgram == StartProgram.IsOsOptimizer) // "% от депозита"
            {
                return Math.Round(tab.Portfolio.ValueCurrent * (volume / 100) / tab.PriceBestAsk / tab.Security.Lot, 12);
            }
            else
            {
                return volume;
    ...
    public class TradeModuleThreeSoldiers
    {
        ...
        public StrategyParameterString ExtraExitType;
        public StrategyParameterDecimal ExtraExitVolaPercentProfit;
        ...
        public TradeModuleThreeSoldiers(BotPanel bot, int moduleNum)
        {
            ...
            ExtraExitType = bot.CreateParameter("ExtraExitType", "Off", new[] { "Off", "Profit vola percent" }, "Soldiers");
            ExtraExitVolaPercentProfit = bot.CreateParameter("Profit vola percent", 100, 1.0m, 500, 4, "Soldiers");
            ...
            BotTabSimple tab, BotTabScreener screener)
        {
            try
            {
                ...
                else
                {
                    ...
                    LogicClosePositionByExtraExit(_candlesTradeSec, tab, securitiesTradeSettings);
                }
            }
            catch (Exception error)
            {
        ...
        private void LogicClosePositionByExtraExit(List<Candle> candles, BotTabSimple tab, SecuritiesTradeSettings settings)
        {
            if (ExtraExitType.ValueString == "Off")
            {
                return;
            }
            ...
            for (int i = 0; openPositions != null && i < openPositions.Count; i++)
            {
                ...
                if (openPositions[i].ProfitOrderPrice != 0)
                {
                    continue;
                }
                ...
                if (openPositions[i].Direction == Side.Buy)
                {
                    ...
                    decimal priceStop = _lastPrice + _lastPrice * ((heightPattern * (ExtraExitVolaPercentProfit.ValueDecimal / 100)) / 100);

                    if (OrdersType.ValueString == "Limit")
                    {
                        tab.CloseAtProfit(openPositions[i], priceStop, priceStop - priceStop * (SlippagePercent.ValueDecimal / 100));
                    }
                    else //if (OrdersType.ValueString == "Market")
                    {
                        tab.CloseAtProfitMarket(openPositions[i], priceStop);
                    }
                }
                else
                {
        ...
        private SecuritiesTradeSettings GetMyVolaSetting(List<Candle> candles, BotTabSimple tab)
        {
            ...
            for (int i = 0; i < TradeSettings.Count; i++)
            {
                if (TradeSettings[i].SecName == tab.Security.Name &&
                    TradeSettings[i].SecClass == tab.Security.NameClass)
                {
